...
BirtNavigationBar.prototype = Object.extend( new AbstractUIComponent( ),
{
	...
	__cb_bind : function( data )
	{
		...
		document.getElementById("gotoPage").value = oPageNumbers[0].firstChild.data;
		...
		if(document.getElementById("show_chart")) {
			oImgs[8].style.cursor = (!isFirstPage)? "pointer" : "default";
			oImgs[10].style.cursor = (!isFirstPage)? "pointer" : "default";
			oImgs[11].style.cursor = (!isLastPage)? "pointer" : "default";
			oImgs[12].style.cursor = (!isLastPage)? "pointer" : "default";
			
			oImgs[8].src = this._getImageFileName( this._IMAGE_FIRST_PAGE, isFirstPage );
			oImgs[10].src = this._getImageFileName( this._IMAGE_PREVIOUS_PAGE, isFirstPage );
			oImgs[11].src = this._getImageFileName( this._IMAGE_NEXT_PAGE, isLastPage );
			oImgs[12].src = this._getImageFileName( this._IMAGE_LAST_PAGE, isLastPage );	
		} else {
			oImgs[6].style.cursor = (!isFirstPage)? "pointer" : "default";
			oImgs[7].style.cursor = (!isFirstPage)? "pointer" : "default";
			oImgs[9].style.cursor = (!isLastPage)? "pointer" : "default";
			oImgs[10].style.cursor = (!isLastPage)? "pointer" : "default";
			
			oImgs[6].src = this._getImageFileName( this._IMAGE_FIRST_PAGE, isFirstPage );
			oImgs[7].src = this._getImageFileName( this._IMAGE_PREVIOUS_PAGE, isFirstPage );
			oImgs[9].src = this._getImageFileName( this._IMAGE_NEXT_PAGE, isLastPage );
			oImgs[10].src = this._getImageFileName( this._IMAGE_LAST_PAGE, isLastPage );
		}
	},
	...
	__cb_installEventHandlers : function( )
	{
		...
		var select_page = this.__instance.getElementsByTagName('select');
		Event.observe( select_page[0], 'change', this.__select_page.bindAsEventListener( this ), false );
	},
	
	/**
	 * 选中页数
	 */
	__select_page : function(event) {
		var num = document.getElementById("select_page").options[window.document.getElementById("select_page").selectedIndex].text;
		parent.__common.changePagerecord(num);
	},
